...
def findGPS(data_file,coordList):
    # returns a list of GPS coords scraped from the input file
    print("scraping data from " + data_file)
    ...
    for line in data:
        if line[:13] == "GPS Location:":
            ...
            if convertISO2DecimalDegrees(lon) != 696970.15 and convertISO2DecimalDegrees(lat) != 696970.15:
                coordList.append([convertISO2DecimalDegrees(lon),convertISO2DecimalDegrees(lat)])
    return coordList
...
def findData(data_file,log_file):
    ...
    pointMarker = Point((convertISO2DecimalDegrees(lat),convertISO2DecimalDegrees(lon)))
    with open('Slocum_tricks/slocum.geojson', 'a') as f:
        json.dump(pointMarker,f)
    #sendFile2FTP('slocum.geojson')
    #update()
...
def retroactive_local():
    coordList = []
    filez = glob.glob("/home/fred/Desktop/Projects/quickGlider/logs/*.log")
    filez.sort(key=os.path.getmtime)
    # find the last line, change to a file you have
    list_file_reader = open ( 'list',"r" )
    for line in filez:
        coordList = findGPS(line,coordList)

    line = LineString(coordList)
...
def main():
    ...
    # Retrieve the file
    retrieve(address, output) ## Retrieve file from the FTP server
